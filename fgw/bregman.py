...
def fgw(
    ...
    if solver in ['PGD', 'PPA']:
        ...
        return fgw_bregman(
            M, C1, C2, p=p, q=q, loss_fun=loss_fun, epsilon=epsilon,
            symmetric=symmetric, alpha=alpha, G0=G0, max_iter=max_iter,
            tol=tol, verbose=verbose, log=log,
        )
    else:
...
def fgw_projected(
    ...
    while (err > tol and cpt < max_iter):
        ...
        if symmetric:
            ...
            tens = (alpha * 0.5) * (gwggrad(constC, hC1, hC2, T) + gwggrad(constCt, hC1t, hC2t, T)) + (1 - alpha) * M
    ...
    if abs(torch.sum(T) - 1) > 1e-5:
        warnings.warn("Solver failed to produce a transport plan. You might want to increase the regularization parameter `epsilon`.")
    if log:
...
def fgw_bregman(
    ...
    if marginal_loss:
        if symmetric:
            ...
            def df(T):
                return (alpha * 0.5) * (gwggrad(constC, hC1, hC2, T) + gwggrad(constCt, hC1t, hC2t, T)) + (1 - alpha) * M
