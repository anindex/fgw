...
def sinkhorn_knopp(a, b, M, reg, numItermax=1000, stopThr=1e-9,
    ...
    for ii in range(numItermax):
        ...
        if warn:
            warnings.warn("Sinkhorn did not converge. You might want to increase the number of iterations `numItermax` or the regularization parameter `reg`.")
    if log:
        log['niter'] = ii
        log['u'] = u
        log['v'] = v
...
def greenkhorn(a, b, M, reg, numItermax=10000, stopThr=1e-9, verbose=False,
    ...
    if warmstart is None:
        u = torch.full((dim_a,), 1. / dim_a).to(M)
        v = torch.full((dim_b,), 1. / dim_b).to(M)
    else:
        u, v = torch.exp(warmstart[0]), torch.exp(warmstart[1])
    G = u[:, None] * K * v[None, :]
...
def sinkhorn_stabilized(a, b, M, reg, numItermax=1000, tau=1e3, stopThr=1e-9,
    ...
    if n_hists:
        ...
        u, v = torch.full((dim_a,), 1. / dim_a).to(M), torch.full((dim_b,), 1. / dim_b).to(M)
    ...
    if log:
        ...
        if n_hists:
            res = torch.stack([
                torch.sum(get_Gamma(alpha, beta, u[:, i], v[:, i]) * M)
